...
if(!isPasswordCorrect){
 throw new ApiResponse(400,"old is password is Invalid")
}
...
const updateUserAvatar=asyncHandler(async(req,res)=>{
     ...
     if(!avatarLocalPath){
          throw new ApiResponse(400,"Avatar local file is required")
     }
    const avatar= await  uploadOnCloudinary(avatarLocalPath)
     if(!avatar.url){
          throw new ApiResponse(400,"Error while uploading avatar on Cloudinary")
     }
     const user=await User.findByIdAndUpdate(
          req.user?.id,
          {
...
const updateCoverImage=asyncHandler(async(req,res)=>{
     ...
     if(!coverImageLocalPath){
          throw new ApiResponse(400,"cover image is required")
     }
     const coverImage=await uploadOnCloudinary(coverImageLocalPath)
     if(!coverImage.url){
          throw new ApiResponse(400,"Error while uploading cover Image on cloudinary")
     }
     const user=await User.findByIdAndUpdate(
          req.user?.id,
          {
...
export {registerUser,loginUser,logoutUser,refreshAccessToken,changeCurrentPassword,getCurrentUser,updateAccountDetails,updateUserAvatar,updateCoverImage,getuserChannerlProfile}